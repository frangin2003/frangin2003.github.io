const strategies = [
  ...
  // Emails
  {
    name: "Email",
    match: (term) =>
      ["Email"].every((tag) => term.tags.includes(tag)) ||
      (["Url"].every((tag) => term.tags.includes(tag)) &&
        term.text.includes("@")),
    execute: (term) => {
  ...
  {
    name: "Url",
    match: (term) => ["Url"].some((tag) => term.tags.includes(tag)),
    execute: (term) => {
      term.backgroundColor = "#FFDAB9"; // Pastel peach
      term.placeholder = window.faker.internet.url();
    },
  },
  // Dates
  {
    name: "Weekday",
    match: (term) =>
      /^(Mon|Tue|Tues|Wed|Wednes|Thu|Thurs|Fri|Sat|Satur|Sun)(day)?$/i.test(
        term.text
      ),
    execute: (term) => {
  ...
  },
  {
    ...
    execute: (term) => {
      ...
      const fakeDate = new Date(
        window.faker.date.anytime({
          refDate: new Date(term.text),
        })
      );
      try {
        term.placeholder =
          luxon.DateTime.fromJSDate(fakeDate).toFormat(dateFormat);
      } catch (error) {
        console.error(
          `Error generating fake date: ${fakeDate} ${dateFormat}`,
          error
        );
      }
    },
  ...
  // Money
  {
    name: "Value",
    match: (term) =>
      ["Value", "TextValue", "Multiple", "Cardinal"].every((tag) =>
  ...
  // Addresses
  {
    name: "BuildingNumber",
    match: (term) =>
      ["Address", "Place", "Cardinal", "NumericValue"].every((tag) =>
        term.tags.includes(tag)
      ),
    execute: (term) => {
  ...
  },
  {
    name: "City",
    match: (term) =>
      ["Place", "City"].every((tag) => term.tags.includes(tag)),
    execute: (term) => {
      term.backgroundColor = "#F08080"; // Pastel red
  ...
  },
  {
    name: "Street",
    match: (term) =>
      ["Address", "Place"].every((tag) => term.tags.includes(tag)),
    execute: (term) => {
      term.backgroundColor = "#F08080"; // Pastel red
  ...
  },
  {
    name: "FirstName",
    match: (term) =>
      ["Person", "FirstName"].every((tag) => term.tags.includes(tag)),
    execute: (term) => {
  ...
  },
  {
    name: "LastName",
    match: (term) =>
      ["Person", "LastName"].every((tag) => term.tags.includes(tag)),
    execute: (term) => {
      term.backgroundColor = "#FFC0CB"; // Pastel pink
  ...
  // Password
  {
    ...
    match: (term) =>
      ...
      new Fuse([TERMS_BY_ID.get(term.previousId).text], {
        threshold: 0.3,
      }).search("password").length > 0,
    execute: (term) => {
